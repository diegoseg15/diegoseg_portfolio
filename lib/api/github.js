...
const fallbackToLocal = () => {
  console.warn("GitHub API bloqueada. Usando datos locales.");
...
export async function publicRepos() {
  try {
    ...
    console.error("Error general GitHub:", error);
    return publicProjects; // fallback final
  }
}
...
export async function getRepoReadme(slug) {
  try {
    ...
    const repoRes = await fetch(
      `https://api.github.com/repos/diegoseg15/${slug}`,
      {
        headers: {
    ...
    // README GitHub
    const readmeRes = await fetch(
      `https://api.github.com/repos/diegoseg15/${slug}/readme`,
      {
        headers: {
          Accept: "application/vnd.github.v3+json",
        },
